...
export default function AIDetection() {
  ...
  const handlePredictImageOnly = async () => {
    ...
    try {
      ...
      setError('Failed to analyze image using YOLOv8 object detector.');
    } finally {
  ...
  const handleAnalyzeAndDispatchImage = async () => {
    ...
    try {
      const res = await aiService.analyzeAndDispatchImage(formData);
      setImageDispatchResult(res);
      setImagePrediction(res.prediction);
    } catch (err) {
      console.error('Image dispatch error:', err);
      const msg = err.response?.data?.detail || 'Failed to dispatch image emergency alert.';
      setError(msg);
    } finally {
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
                ...
                {voiceDispatchResult && (
                  <>
                    ...
                    <div>
                      ...
                      <div className="w-full h-2.5 bg-slate-900 rounded-full overflow-hidden border border-slate-800">
                        <motion.div initial={{ width: 0 }} animate={{ width: `${voiceDispatchResult.prediction.confidence * 100}%` }} transition={{ duration: 0.8 }} className="h-full bg-gradient-to-r from-emerald-500 to-green-400 rounded-full" />
                      </div>
                    </div>
      ...
      {activeTab === 'image' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* Right Column: YOLOv8 Result Panel */}
          <div className="space-y-6">
            {imagePrediction ? (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-card rounded-2xl p-6 border border-slate-800 space-y-5">
                <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {imagePrediction.detected_objects.map((obj, idx) => (
                      <span key={idx} className="px-3 py-1 rounded-xl bg-slate-900 border border-slate-800 text-xs text-amber-300 font-semibold flex items-center gap-1.5">
                        <Tag className="w-3 h-3 text-amber-400" />
                        <span>{obj.label} ({(obj.confidence * 100).toFixed(0)}%)</span>
                      </span>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  <span className="text-xs text-slate-400">Predicted Category</span>
                  <div className="mt-1 flex items-center gap-2">
                    <span className="text-2xl">{getCategoryBadge(imagePrediction.prediction).icon}</span>
                    <span className={`px-3 py-1 rounded-xl text-lg font-extrabold border ${getCategoryBadge(imagePrediction.prediction).color}`}>
                      {imagePrediction.prediction} Emergency
                    </span>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between text-xs mb-1.5">
                    <span className="text-slate-400 font-medium">Confidence Score</span>
                    <span className="text-emerald-400 font-bold font-mono">
                      {(imagePrediction.confidence * 100).toFixed(1)}%
                    </span>
                  </div>
                  <div className="w-full h-2.5 bg-slate-900 rounded-full overflow-hidden border border-slate-800">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${imagePrediction.confidence * 100}%` }} transition={{ duration: 0.8 }} className="h-full bg-gradient-to-r from-emerald-500 to-green-400 rounded-full" />
                  </div>
                </div>
                ...
                {imageDispatchResult && (
                  <div className="p-4 rounded-xl bg-emerald-950/30 border border-emerald-500/30 space-y-3">
                    <div className="flex items-center gap-2 text-emerald-400 text-xs font-bold">
                      <CheckCircle2 className="w-4 h-4" />
                      <span>Vision Alert Dispatched via ntfy.sh</span>
                    </div>
                    <p className="text-slate-300 text-xs">
                      Event ID #{imageDispatchResult.event.id} logged in database with modality "Image".
                    </p>
                    <a href={imageDispatchResult.event.google_maps_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-400 hover:text-emerald-300 transition-colors">
                      <span>Google Maps Location</span>
                      <ExternalLink className="w-3.5 h-3.5" />
                    </a>
                  </div>
